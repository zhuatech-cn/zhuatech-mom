...
import cn.zhuatech.mom.service.MomService;
...
public class MomController {
    private final MomService service; public MomController(MomService service){this.service=service;}
    @GetMapping("/public/about") ApiResponse<Map<String,Object>> about(){return ApiResponse.ok(service.about());}
    @GetMapping("/catalog") ApiResponse<MomService.CatalogView> catalog(){return ApiResponse.ok(service.catalog());}
    @GetMapping("/dashboard") ApiResponse<MomService.Dashboard> dashboard(){return ApiResponse.ok(service.dashboard());}
    ...
    @PostMapping("/records/{id}/comments") ApiResponse<BusinessRecord> comment(@PathVariable Long id,@Valid @RequestBody MomService.CommentRequest request){return ApiResponse.ok(service.comment(id,request));}
    @GetMapping("/sla-summary") ApiResponse<MomService.SlaSummary> sla(){return ApiResponse.ok(service.slaSummary());}
    @PostMapping("/records") ApiResponse<BusinessRecord> create(@Valid @RequestBody MomService.RecordRequest request){return ApiResponse.ok(service.create(request));}
    @PutMapping("/records/{id}") ApiResponse<BusinessRecord> update(@PathVariable Long id,@Valid @RequestBody MomService.RecordRequest request){return ApiResponse.ok(service.update(id,request));}
    @PostMapping("/records/{id}/actions") ApiResponse<BusinessRecord> action(@PathVariable Long id,@Valid @RequestBody MomService.ActionRequest request){return ApiResponse.ok(service.action(id,request));}
